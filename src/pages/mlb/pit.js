import React from 'react';
import Header from '../../components/Header';
import Footer from '../../components/Footer';
import './mlb-pitchers.css';

const MLBTeamPage = () => {
  const pitchers = [
    {
      name: 'Paul Skenes',
      number: 30,
      profileLink: '/mlb/pirates/paul-skenes',
      headshot: 'https://a.espncdn.com/combiner/i?img=/i/headshots/mlb/players/full/4719507.png&w=350&h=254'
    },
    {
      name: 'Mitch Keller',
      number: 23,
      profileLink: '/mlb/pirates/mitch-keller',
      headshot: 'https://a.espncdn.com/combiner/i?img=/i/headshots/mlb/players/full/33722.png&w=350&h=254'
    },
    {
      name: 'Bubba Chandler',
      number: 36,
      profileLink: '/mlb/pirates/bubba-chandler',
      headshot: 'https://a.espncdn.com/combiner/i?img=/i/headshots/mlb/players/full/4683325.png&w=350&h=254'
    },
    {
      name: 'Braxton Ashcraft',
      number: 35,
      profileLink: '/mlb/pirates/braxton-ashcraft',
      headshot: 'https://a.espncdn.com/combiner/i?img=/i/headshots/mlb/players/full/41282.png&w=350&h=254'
    },
    {
      name: 'Carmen Mlodzinski',
      number: 50,
      profileLink: '/mlb/pirates/carmen-mlodzinski',
      headshot: 'https://a.espncdn.com/combiner/i?img=/i/headshots/mlb/players/full/4298378.png&w=350&h=254'
    }


  ];

  return (
    <>
      <Header />
      <main className="team-container">
        <h1>Pittsburgh Pirates — Starting Pitchers</h1>
        <p>Click a pitcher to open their player page.</p>

        <div className="pitcher-grid">
          {pitchers.map((p) => (
            <a className="pitcher-card" href={p.profileLink} key={p.name}>
              <div className="pitcher-photo">
                {p.headshot ? (
                  <img src={p.headshot} alt={`${p.name} headshot`} loading="lazy" />
                ) : (
                  <div className="pitcher-photo--ph" aria-hidden="true" />
                )}
              </div>
              <div className="pitcher-info">
                <div className="pitcher-name">{p.name}</div>
                {p.number && <div className="pitcher-num">#{p.number}</div>}
              </div>
            </a>
          ))}
        </div>
      </main>
      <Footer />
    </>
  );
};

export default MLBTeamPage;
